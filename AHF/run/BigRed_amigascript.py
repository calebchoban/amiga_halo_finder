...
if len(sys.argv)>5:
	...
	num_task_per_node = int(sys.argv[8])
	num_OMP_THREADS = int(sys.argv[9])
elif len(sys.argv)>4:
	starno = 0
	NSnap = 600
...
while(count <= Nsnap):
    ...
    if count<10:
        ...
        strno='0'+str(count)
    print('Doing ',strno)
    mycommand = 'srun --cpus-per-task='+str(num_OMP_THREADS)+' --ntasks-per-node='+str(num_task_per_node)+' --nodes='+str(num_nodes)+' '+amiga_dir+'AHF AHF.input'+strno
    print(mycommand)
    os.system(mycommand)
